...
const Header: FC<HeaderProps> = ({ title, setTitle, onReset, onExport, theme, toggleTheme }) => {
  return (
    <header className="flex items-center justify-between px-4 py-3 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-b border-gray-200/80 dark:border-gray-700/80 shadow-sm z-10 print:hidden">
      ...
      {/* Document Title */}
      <div className="flex-1 max-w-xl min-w-0 mx-2 sm:mx-4">
        <input
          type="text"
          value={title}
          ...
          className="w-full px-3 sm:px-4 py-2 text-center bg-gray-50 dark:bg-gray-900/50 border border-transparent hover:border-gray-300 dark:hover:border-gray-600 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-gray-700 dark:text-gray-200 font-medium transition-all placeholder:text-gray-400"
        />
      </div>

      {/* Actions */}
      <div className="flex items-center gap-0.5 sm:gap-1.5">
        {/* GitHub Link - hidden on the smallest screens to avoid crowding */}
        <a
          href="https://github.com/Akm592/markdown-to-pdf"
          target="_blank"
          rel="noopener noreferrer"
          className="hidden min-[400px]:inline-flex p-2.5 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-all"
        ...
        <div className="hidden min-[400px]:block w-px h-6 bg-gray-200 dark:bg-gray-700 mx-1" />
